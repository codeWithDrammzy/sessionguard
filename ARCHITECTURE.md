...
| **`KeystrokeDynamics`** | Per-keystroke biometrics: the three legacy aggregate means (mean hold, mean interval, characters-per-minute) PLUS rhythm-shape features (hold/interval std-dev, longest pause, backspace count) + `login_pin_failures`; NULL for USSD (no keyboard API). Historical rows form the typing baseline via a causal z-score; shape features are optional per-row and simply add dimensions when present. |
...
- HEAD `d0007fa` (docs §14 follow-up); the submission baseline itself is `b5c459f` "Finalize submission baseline: balance checks never scored (read-only short-circuit), family_sharing preset time-of-day pin, 30-test suite, regenerated 250/2285 dataset + retrained model bundle, refreshed eval numbers and docs" — both pushed; working tree clean, `main == origin/main`.
