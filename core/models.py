...
class BankUser(models.Model):
    ...
    # Demo banking fields (Control Room / customer app): a starting balance
    # for new self-service signups and a display name. Real deployments
    # would source these from core banking, not the fraud layer.
    first_name = models.CharField(
        max_length=60,
        blank=True,
        default="",
        help_text="Customer display name for the demo bank app.",
    )
    phone_number = models.CharField(
        ...
        default="",
        db_index=True,
        help_text=("Demo signup handle. Uniqueness is enforced at the "
                   "signup endpoint (a DB constraint is impossible here: "
                   "the 250 seeded dataset customers predate this field "
                   "and share the blank default)."),
    )
...
class Transaction(models.Model):
    ...
    # --- Demo banking fields (customer app / USSD simulator) ---------------
    # reference: human-facing transfer receipt number, set when a transfer
    #   is actually committed (approved) or held (challenged).
    # outcome: what the risk engine decided. Dataset rows generated by
    #   inject_attacks keep the implicit "approve" default only where they
    #   predate this field; live bank-app rows always carry the verdict.
    OUTCOME_APPROVE = "approve"
    OUTCOME_CHALLENGE = "challenge"
    OUTCOME_BLOCK = "block"
    OUTCOME_CHOICES = [
        (OUTCOME_APPROVE, "Approved"),
        (OUTCOME_CHALLENGE, "Challenged (verification)"),
    ...
    reference = models.CharField(
        ...
        blank=True,
        default="",
        help_text="Receipt / hold reference, e.g. SG-9F2C41AB.",
    )
    outcome = models.CharField(
        max_length=12,
        choices=OUTCOME_CHOICES,
        default=OUTCOME_APPROVE,
        help_text="Risk decision attached to this attempted transfer.",
    )
